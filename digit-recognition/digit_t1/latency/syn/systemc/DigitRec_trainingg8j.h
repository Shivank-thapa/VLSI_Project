...
#ifndef _DigitRec_trainingg8j_H_
#define _DigitRec_trainingg8j_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "DigitRec_trainingg8j_memcore.h"

SC_MODULE(DigitRec_trainingg8j) {
    static const unsigned int DataWidth    = 256;
...
protected:
    ...
    DigitRec_trainingg8j_memcore* DigitRec_trainingg8j_memcore_U;
...
public:
    ...
    SC_CTOR(DigitRec_trainingg8j) : m_trace_file(0) {
        SC_METHOD(proc_memcore_addr);
        sensitive << i_address0 << t_address0 << iptr << tptr;

        DigitRec_trainingg8j_memcore_U = new DigitRec_trainingg8j_memcore("DigitRec_trainingg8j_memcore_U");
        DigitRec_trainingg8j_memcore_U->clk      ( clk );
        DigitRec_trainingg8j_memcore_U->reset    ( reset );
        DigitRec_trainingg8j_memcore_U->ce0      ( i_ce0 );
        DigitRec_trainingg8j_memcore_U->we0      ( i_we0 );
        DigitRec_trainingg8j_memcore_U->address0 ( memcore_iaddr );
        DigitRec_trainingg8j_memcore_U->d0       ( i_d0 );
        DigitRec_trainingg8j_memcore_U->q0       ( i_q0 );

        DigitRec_trainingg8j_memcore_U->ce1      ( t_ce0 );
        DigitRec_trainingg8j_memcore_U->we1      ( t_we0 );
        DigitRec_trainingg8j_memcore_U->address1 ( memcore_taddr );
        DigitRec_trainingg8j_memcore_U->d1       ( t_d0 );
        DigitRec_trainingg8j_memcore_U->q1       ( t_q0 );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "DigitRec_trainingg8j.trace");
            // ports
