...
#ifndef _DigitRec_results_H_
#define _DigitRec_results_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "DigitRec_results_memcore.h"

SC_MODULE(DigitRec_results) {
...
protected:
    ...
    DigitRec_results_memcore* DigitRec_results_memcore_U;
...
public:
    ~DigitRec_results();
    SC_CTOR(DigitRec_results) : m_trace_file(0) {
        SC_METHOD(proc_memcore_addr);
        sensitive << i_address0 << t_address0 << iptr << tptr;

        DigitRec_results_memcore_U = new DigitRec_results_memcore("DigitRec_results_memcore_U");
        DigitRec_results_memcore_U->clk      ( clk );
        DigitRec_results_memcore_U->reset    ( reset );
        DigitRec_results_memcore_U->ce0      ( i_ce0 );
        DigitRec_results_memcore_U->we0      ( i_we0 );
        DigitRec_results_memcore_U->address0 ( memcore_iaddr );
        DigitRec_results_memcore_U->d0       ( i_d0 );
        DigitRec_results_memcore_U->q0       ( i_q0 );

        DigitRec_results_memcore_U->ce1      ( t_ce0 );
        DigitRec_results_memcore_U->we1      ( t_we0 );
        DigitRec_results_memcore_U->address1 ( memcore_taddr );
        DigitRec_results_memcore_U->d1       ( t_d0 );
        DigitRec_results_memcore_U->q1       ( t_q0 );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "DigitRec_results.trace");
            // ports
